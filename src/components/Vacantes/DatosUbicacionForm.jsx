...
export function DatosUbicacionForm() {
  const { datosVacante, errors, handleInputChange, setDatosVacante } =
    useRegisterVacante();
  ...
  return (
    <Flex gap="1rem" direction="column" justify="space-between">
      ...
      <Stack>
        <Text>
          ¿La ubicación de la vacante es la misma que la de la empresa?
        </Text>
        <Flex justifyContent="center" gap="8rem">
          <Checkbox isChecked={isCheck} onChange={handleCheckboxChange}>
            Sí
          </Checkbox>
          <Checkbox isChecked={!isCheck} onChange={handleCheckboxChange}>
            No
      ...
      {!isCheck && (
        ...
        >
          <GridItem>
            ...
            {errors.municipio && (
              <FormHelperText>
                El campo de municipio es obligatorio
              </FormHelperText>
            )}
          </GridItem>
